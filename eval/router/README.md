...
| 2 | 排除规则 | 从 agent_info【注意事项】提取硬约束，缩小候选 |
| 3 | 反向校准 | 新规则上线前验证不破坏已有正确 case |
| 4 | Curator 四操作 | ADD/MODIFY/DELETE/KEEP，LLM自主管理经验生命周期 |
...
│   └── curator.py             # Curator prompt (经验增删改留)
├── preprocess/
...
```
## EXCLUSION THRESHOLDS        ← 排除约束 (预生成)
## ROUTING STRATEGIES          ← 策略规则 (ACE Curator 增删改留)
## COMMON MISTAKES             ← 常见错误 (ACE Curator 增删改留)
```
